...
class LadderBuy:
    ...
    def activate(
        ...
        if ticker in self._state:
            logger.info(f"[{self.account.name}] {ticker}: ladder already active (local state)")
        ...
        try:
            ...
            if open_orders:
                ...
                return None
            position = self.trading.get_open_position(ticker)
            if position:
                logger.info(f"[{self.account.name}] {ticker}: open position already on Alpaca — skipping ladder, trailing stop should handle")
                return None
        except Exception:
            pass  # no position found = clean to proceed
